...
void pum_display(pumitem_T *array, int size, int selected, bool array_changed, int cmd_startcol)
{
  ...
  if (!parse_completepopup(&fconfig, NULL)) {
    return;
  }
...
void pum_redraw(void)
{
  ...
  WinConfig fconfig = WIN_CONFIG_INIT;
  int border_adj[4];
  if (!parse_completepopup(&fconfig, border_adj)) {
    return;
  }
...
bool parse_completepopup(WinConfig *fconfig, int *border_adj)
{
  const char *p = p_cpp;
  Error err = ERROR_INIT;
  bool find_border = border_adj == NULL;
